...
std::string AIHelper::chat(int userId,std::string userName, std::string sessionId, std::string userQuestion, std::string modelType) {
    ...
    //说明支持MCP
    AIConfig config;
    config.loadFromFile("/home/ros/lib/CppAIWeb/AIApps/ChatServer/resource/config.json");
...
json AIHelper::executeCurl(const json& payload) {
    ...
    std::cout << "[LocalLLM] URL: " << strategy->getApiUrl() << std::endl;
    std::cout << "[LocalLLM] Payload: " << payloadStr << std::endl;
                                ...
                                const std::function<void(const std::string &)> &onChunk)
{
    ...
    wd.onChunk = onChunk;
    wd.strategy = strategy.get();

    // 前缀 + 强制无捕获 lambda 退化为函数指针；直接把 lambda 传给可变参数的
    // curl_easy_setopt 不会触发该转换，会把闭包对象当函数指针传进去（未定义行为），
    // 导致 WRITEFUNCTION 未生效、curl 回退到默认 fwrite 并把 WRITEDATA 当 FILE* 写 → 崩溃
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION,
        +[](void *contents, size_t size, size_t nmemb, void *userp) -> size_t {
            auto *d = static_cast<WriteData *>(userp);
            size_t total = size * nmemb;
            for (size_t i = 0; i < total; ++i) {
                char c = static_cast<char *>(contents)[i];
                if (c == '\n') {
                    if (!d->lineBuffer.empty()) {
                        d->strategy->parseResponseStreaming(d->lineBuffer, d->onChunk);
                        d->lineBuffer.clear();
                    }
                } else {
                    d->lineBuffer.push_back(c);
                }
            }
            return total;
        });
    ...
    // 同步执行，立即触发回调（libcurl 接收到每个 TCP 包时回调一次）
    CURLcode res = curl_easy_perform(curl);
    curl_slist_free_all(headers);
    curl_easy_cleanup(curl);

    // 处理最后一行（如果响应末尾没有 \n）
    if (res == CURLE_OK && !wd.lineBuffer.empty()) {
        wd.strategy->parseResponseStreaming(wd.lineBuffer, wd.onChunk);
    }
...
// 构建流式请求体：复用策略的 buildRequest（基于完整历史），再加 stream:true 让上游以 SSE 返回
